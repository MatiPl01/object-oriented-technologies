...
public class PhotoSerializer {
    ...
    private static final Logger log =
            Logger.getLogger(PhotoSerializer.class.getName());
    ...
    private void createLibraryDirectory() throws IOException {
        ...
        if (!photoLibraryDir.isDirectory()) {
            throw new IOException(
                    "This is not valid photo library directory path or directory could not be created: " +
                    photoLibraryPath);
        }
    }
    ...
    public void savePhoto(Photo photo) {
        ...
        try {
            ImageIO.write(
                    SwingFXUtils.fromFXImage(photo.getPhotoData(), null),
                    "png",
                    new File(getPhotoPath(photo))
            );
        } catch (IOException e) {
            e.printStackTrace();
        }
